...
use std::{fmt, io, mem, net::SocketAddr, sync::Arc};

use super::{into_io_error, Bind};
use crate::{
    fiber::{self, Context},
    io::poll::{EventedHandle, Interest, Register},
...
impl TcpListener {
    /// Makes a future to create a new `TcpListener` which will be bound to the
    /// specified address.
    pub fn bind(addr: SocketAddr) -> TcpListenerBind {
        TcpListenerBind(Bind::Bind(addr, MioTcpListener::bind))
    }

    /// Makes a stream of the connections which will be accepted by this
    /// listener.
...
///
/// This is created by calling `TcpListener::bind` function. It is permitted to
...
pub struct TcpListenerBind(
    Bind<fn(&SocketAddr) -> io::Result<MioTcpListener>, MioTcpListener>,
);
...
///
/// This is created by calling `TcpListener::incoming` method. It is permitted
/// to move the future across fibers.
///
/// # Panics
///
        ...
        loop {
            if let Some(mut monitor) = self.0.monitor.take() {
                if let Async::NotReady =
                    monitor.poll().map_err(into_io_error)?
                {
                ...
                match self.0.handle.inner().accept() {
                    Ok((stream, addr)) => {
                        let register =
                            |mut c: Context| c.poller().register(stream);
                        let future =
                            assert_some!(fiber::with_current_context(register));
                    ...
                    Err(e) => {
                        if e.kind() == io::ErrorKind::WouldBlock {
                            self.0.monitor =
                                Some(self.0.handle.monitor(Interest::Read));
                        } else {
                            return Err(e);
                        }
                    },
...
///
/// This is produced by `Incoming` stream. It is permitted to move the future
/// across fibers.
///
/// # Panics
///
...
/// # Note
///
/// Non blocking mode is always enabled on this socket. Roughly speaking, if an
/// operation (read or write) for a socket would block, it returns the
/// `std::io::ErrorKind::WouldBlock` error and current fiber is suspended until
/// the socket becomes available. If the fiber has multiple sockets (or other
/// objects which may block), it will be suspended only the case that all of
/// them are unavailable.
///
/// To handle read/write operations over TCP streams in
/// [futures](https://github.com/alexcrichton/futures-rs) style, it is preferred
/// to use external crate like
        ...
        loop {
            if let Some(mut monitor) = self.monitor(interest).take() {
                ...
                match result {
                    Err(e) => {
                        if e.kind() == io::ErrorKind::WouldBlock {
                            *self.monitor(interest) =
                                Some(self.handle.monitor(interest));
                        } else {
                            return Err(e);
                        }
                    },
...
///
/// This is created by calling `TcpStream::connect` function. It is permitted to
/// move the future across fibers.
///
/// # Panics
///
                ...
                match stream.peer_addr() {
                    ...
                    Err(e) => {
                        ...
                        if e.kind() == io::ErrorKind::NotConnected {
                            let retry = stream
                                .start_monitor_if_needed(Interest::Write)?;
