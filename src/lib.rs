...
//! This is a library to execute a number of lightweight asynchronous tasks
//! (a.k.a, fibers).
//!
//! Note that `fibers` heavily uses [futures][futures] to represent asynchronous
//! task. If you are not familiar with it, we recommend that you refer the
//! `README.md` and `TUTORIAL.md` of [futures][futures] before reading the
//! following.
//!
...
//! However, its existence is hidden from the user, so you do not usually have
//! to worry about it.
//!
...
//! ---
//!
//! `Future` is an excellent way to represent asynchronous task. It is
//! intuitive, easily composed with other futures to represent a complicated
//! task, without runtime overhead. But, there is a remaining problem that "How
//! to efficiently execute (possibility a very large amount of) concurrent
//! tasks?". `fibers` is an answer to the problem.
//!
//! Conceptually, the responsibility of `fibers` is very simple. It represents
//! an asynchronous task (a.k.a., fiber) as a future instance. And there is an
//! executor that takes futures and executes them like following.
//!
//! ```ignore
//! // Creates an executor.
...
//! ```
//!
//! Fibers may be run on different background threads, but the user does not
//! need to notice it. If it runs on machines with a large number of processors,
//! performance will improve naturally.
//!
//! Roughly speaking, if a future returns `Async::NotReady` response to a call
//! of `Future::poll` method, the fiber associated with the future will move
//! into the "waiting" state. Then, it is suspended (descheduled) until any
//! event in which the future is interested happens (e.g., waits until data is
//! arrived on a target TCP socket). Finally, if a future returns `Async::Ready`
//! response, the fiber will be regarded as completed and the executor will drop
//! the fiber.
//!
...
//! [sync](sync/index.html), [io](io/index.html), [time](time/index.html)
//! modules for more details).
//!
//! The main concern of this library is "how to execute fibers". So it is
//! preferred to use external crates (e.g., [`handy_async`][handy_async]) to
//! describe "how to represent asynchronous tasks".
//!
...
//!
//! Other examples are found in "fibers/examples" directory. And you can run an
